...
export default function Sidebar() {
	...
	const guide = config.nav.filter(item =>
		isDocs ? item.content === 'guide' : item.content === 'cli'
	)[0];
		...
		<div class={style.wrapper} data-open={open}>
			...
			<aside class={style.sidebar}>
				<div class={style.sidebarInner}>
					{isDocs && <DocVersion />}
					<SidebarNav items={items} onClick={close} />
				</div>
			</aside>
		</div>
	);
