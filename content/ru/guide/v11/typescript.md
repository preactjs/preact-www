...
`ComponentChildren` — это тип, представляющий все допустимые дочерние элементы Preact. Он включает примитивные типы, такие как `string`, `number` и `boolean`, а также элементы Preact, `null`/`undefined` и массивы всех перечисленных выше типов. Для тех, кто знаком с React, он очень похож на `ReactNode`.
...
<MyHeadingComponent>
	{/* Все эти значения являются допустимыми дочерними элементами */}
	Привет, мир!
...
> **Примечание**: Мы делаем это, чтобы обеспечить более точную типизацию ARIA и доступности для этих элементов, поскольку их ARIA-роли согласно спецификации представлены в виде дискриминируемого объединения типов (например, если `<a>` имеет атрибут `href`, для него допустимы лишь несколько определённых ролей, а если атрибута нет — другой набор ролей). Для этого нам необходимо использовать ключевое слово `type` в TypeScript, однако это лишает возможности расширять тип, поскольку он больше не является обычным интерфейсом. При этом наши типы доступности пересекаются с интерфейсами `Partial...`, поэтому вместо этого мы можем расширять их.
