...
Ключ идентифицирует весь `Fragment` как одну группу при согласовании (reconciliation). Когда группы `Fragment` меняются местами, стабильные ключи помогают Preact сопоставить каждую группу с соответствующей группой из предыдущего рендера, а не сопоставлять их только по позиции. Для `Fragments` с ключами используйте явную форму `<Fragment key={...}>`, поскольку сокращённая запись `<>...</>` не может принимать ключ.
