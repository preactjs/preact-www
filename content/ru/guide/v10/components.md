...
Ключ идентифицирует весь `Fragment` как единую группу во время согласования. Когда группы `Fragment` меняют порядок, стабильные ключи помогают Preact сопоставить каждую группу с соответствующей группой из предыдущего рендера, вместо того чтобы сопоставлять их только по позиции. Для `Fragment` с ключом используйте явную форму `<Fragment key={...}>`, поскольку сокращённая форма `<>...</>` не позволяет указать ключ.
